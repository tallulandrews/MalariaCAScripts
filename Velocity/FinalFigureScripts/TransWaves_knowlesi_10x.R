...
# Add bulk
bulk <- read.table("~/Collaborations/MCA/For10XVelocity/pkbulkcenters.csv", sep=",", header=TRUE)
...
source("~/R-Scripts/Blank_plot.R")
png("Explain_My_Pseudotime2_knowlesi.png", width=7.5, height=6, units="in", res=300)
layout(mat=cbind(c(1,1), c(2,2)), width=c(3.5,1), heights=c(1,1))
par(mar=c(4,4,1,0))
#plot(tmp_xes+fitg[1,1], tmp_yes+fitg[2,1], col=tmp_col[as.numeric(cut(converted, breaks=length(tmp_col)))], pch=16, xlab="PC1", ylab="PC2")
plot(dat_10X$ann[,c("pc1", "pc2")], col=remap_colors(dat_10X$ann$cell_color), pch=16, xlab="PC1", ylab="PC2")
points(tmp_xes[start_cell]+fitg[1,1], tmp_yes[start_cell]+fitg[2,1], col="red", pch=16)
...
lines(c(plot_fit[155,1], fitg[1,1], dat_10X$ann[start_cell,"pc1"]),
      c(plot_fit[155,2], fitg[2,1], dat_10X$ann[start_cell,"pc2"]), col="black", lwd=2)
...
pdf("knowlesi_stats_vs_pseudotime.pdf", width=5*2/3, height=10*2/3)
