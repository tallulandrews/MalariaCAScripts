...
#pdf("noncomp_Three_Spp_Wave_Lines.pdf", width=7, height=9)
...
pb_smoothed <- smooth.spline(bergei_pseudo, bergei_up_s)
bit <- knowlesi_scmap_pseudo$pseudotime>-1
pk_smoothed <- smooth.spline(knowlesi_scmap_pseudo$pseudotime[bit], knowlesi_up_s[bit], spar=0.9)
bit <- falciparum_scmap_pseudo$pseudotime > 0.5 & falciparum_scmap_pseudo$pseudotime < 6.5
pf_smoothed <- smooth.spline(falciparum_scmap_pseudo$pseudotime[bit], falciparum_up_s[bit], spar=0.9)
...
# correlations:
common_pseudo_breaks <- bergei_cell_breaks
pber_common_bins <- cut(bergei_pseudo, breaks=common_pseudo_breaks)
pkno_common_bins <- cut(knowlesi_scmap_pseudo$pseudotime, breaks=common_pseudo_breaks)
pfal_common_bins <- cut(falciparum_scmap_pseudo$pseudotime, breaks=common_pseudo_breaks)
pber_smoothed_waves <- aggregate(bergei_up_s, by=list(pber_common_bins), mean)
pkno_smoothed_waves <- aggregate(knowlesi_up_s, by=list(pkno_common_bins), mean)
pfal_smoothed_waves <- aggregate(falciparum_up_s, by=list(pfal_common_bins), mean)
# force_match
pber_smoothed_waves[,1] <- as.character(pber_smoothed_waves[,1])
pkno_smoothed_waves[,1] <- as.character(pkno_smoothed_waves[,1])
pfal_smoothed_waves[,1] <- as.character(pfal_smoothed_waves[,1])

cor(pkno_smoothed_waves[,2], pber_smoothed_waves[pber_smoothed_waves[,1] %in% pkno_smoothed_waves[,1],2])
# 0.73
cor(pfal_smoothed_waves[,2], pber_smoothed_waves[pber_smoothed_waves[,1] %in% pfal_smoothed_waves[,1],2])
# 0.16 ns
cor(pfal_smoothed_waves[,2], pkno_smoothed_waves[pkno_smoothed_waves[,1] %in% pfal_smoothed_waves[,1],2])
# -0.12 ns

# Individual peaks/toughs
v_lines <- p_breaks
# main peak = 5-6 vs 3->4
#last trough = -2->-1 vs -4->-3
# mid trough -5->-4 vs -7->-6

up_vec <- falciparum_up_s # bergei_up_s, knowlesi_up_s, falciparum_up_s,
pseudo_vec <- falciparum_scmap_pseudo$pseudotime # bergei_pseudo, knowlesi_scmap_pseudo$pseudotime, falciparum_scmap_pseudo$pseudotime
# main peak
hi = up_vec[pseudo_vec > v_lines[6] & pseudo_vec < v_lines[7]]
lo = up_vec[pseudo_vec > v_lines[4] & pseudo_vec < v_lines[5]]
t.test(hi,lo)
# last trough
hi = up_vec[pseudo_vec > v_lines[17] & pseudo_vec < v_lines[18]]
lo = up_vec[pseudo_vec > v_lines[19] & pseudo_vec < v_lines[20]]
t.test(hi,lo)
#mid trough
hi = up_vec[pseudo_vec > v_lines[14] & pseudo_vec < v_lines[15]]
lo = up_vec[pseudo_vec > v_lines[16] & pseudo_vec < v_lines[17]]
t.test(hi,lo)
#mid peak
lo = up_vec[pseudo_vec > v_lines[12] & pseudo_vec < v_lines[13]]
hi = up_vec[pseudo_vec > v_lines[14] & pseudo_vec < v_lines[15]]
t.test(hi,lo)

# Add Sig features to plot
nlines=1
#pdf("noncomp_Three_Spp_Wave_Lines_w_Sig.pdf", width=7, height=9)
lmat <- cbind(c(1:nlines, nlines+1:nspp), c(1:nlines, nlines+1:nspp))
layout(lmat, width=c(1,1), heights=c(rep(0.1, times=nlines), rep(1, times=nspp)))
...
# sig
plot_piecewise(bergei_pseudo, bergei_up_s, splits)

#up_vec <- bergei_up_s 
#pseudo_vec <- bergei_pseudo 
#lines(c(mean(c(v_lines[4], v_lines[5])),mean(c(v_lines[7], v_lines[6]))), 
#      c(mean(up_vec[pseudo_vec > v_lines[4] & pseudo_vec < v_lines[5]]), 
#	mean(up_vec[pseudo_vec > v_lines[6] & pseudo_vec < v_lines[7]])), lty=2, lwd=2, col="red")
#lines(c(mean(c(v_lines[17], v_lines[18])),mean(c(v_lines[19], v_lines[20]))), 
#      c(mean(up_vec[pseudo_vec > v_lines[17] & pseudo_vec < v_lines[18]]), 
#	mean(up_vec[pseudo_vec > v_lines[19] & pseudo_vec < v_lines[20]])), lty=2, lwd=2, col="red")
#lines(c(mean(c(v_lines[14], v_lines[15])),mean(c(v_lines[16], v_lines[17]))), 
#      c(mean(up_vec[pseudo_vec > v_lines[14] & pseudo_vec < v_lines[15]]), 
#	mean(up_vec[pseudo_vec > v_lines[16] & pseudo_vec < v_lines[17]])), lty=2, lwd=2, col="red")
#lines(c(mean(c(v_lines[12], v_lines[13])),mean(c(v_lines[14], v_lines[15]))), 
#      c(mean(up_vec[pseudo_vec > v_lines[12] & pseudo_vec < v_lines[13]]), 
#	mean(up_vec[pseudo_vec > v_lines[14] & pseudo_vec < v_lines[15]])), lty=2, lwd=2, col="red")
...
lines(thing1$x, thing1$y, lwd=2)
# sig
plot_piecewise(knowlesi_scmap_pseudo$pseudotime, knowlesi_up_s, splits)

#up_vec <- knowlesi_up_s 
#pseudo_vec <- knowlesi_scmap_pseudo$pseudotime 
#lines(c(mean(c(v_lines[4], v_lines[5])),mean(c(v_lines[7], v_lines[6]))), 
#      c(mean(up_vec[pseudo_vec > v_lines[4] & pseudo_vec < v_lines[5]]), 
#	mean(up_vec[pseudo_vec > v_lines[6] & pseudo_vec < v_lines[7]])), lty=2, lwd=2, col="red")
#lines(c(mean(c(v_lines[17], v_lines[18])),mean(c(v_lines[19], v_lines[20]))), 
...
lines(thing1$x, thing1$y, lwd=2)
# sig
plot_piecewise(falciparum_scmap_pseudo$pseudotime, falciparum_up_s, splits)

#up_vec <- falciparum_up_s
#pseudo_vec <- falciparum_scmap_pseudo$pseudotime
#lines(c(mean(c(v_lines[4], v_lines[5])),mean(c(v_lines[7], v_lines[6]))), 
#      c(mean(up_vec[pseudo_vec > v_lines[4] & pseudo_vec < v_lines[5]]), 
#	mean(up_vec[pseudo_vec > v_lines[6] & pseudo_vec < v_lines[7]])), lty=2, lwd=2, col="red")
#lines(c(mean(c(v_lines[17], v_lines[18])),mean(c(v_lines[19], v_lines[20]))), 
