...
png("Bergei_SuperPCAWithArrows.png", width=8, height=8, units="in", res=300)
par(mar=c(4,4,1,1))
plot(current_pca, pch=21, bg=dat_10X$ann$cell_color, col="grey65", cex=1.5)
...
for(j in min(binned[,1]):max(binned[,1])) {
	...
	arrows( mean(current_pca[thispts,1]),
		...
		mean(projected_pca[thispts,2]), len=0.07 )
