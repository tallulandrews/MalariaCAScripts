...
my_dtwPlotDensity <- function(d, normalize=FALSE,
    ...
    yaxin=pretty(1:nrow(cm))
    xaxin=pretty(1:ncol(cm))
    axis(2, at=yaxin, labels=yaxin, col="red", col.axis="red", lwd=2)
    axis(1, at=xaxin, labels=xaxin, col="black", col.axis="black", lwd=2)
    contour(cm,x=1:xd,y=1:yd,add=TRUE);
    lines(d$index1,d$index2,col="navy",lwd=4);
    lines(d$index1,d$index2,col="cornflowerblue",lwd=2);
}
...
my_dtwPlotTwoWay <- function(d,xts=NULL,yts=NULL, scalex=1, scaley=1, offset=0,
			ts.type="l",pch=21, 
                        match.indices=NULL,
	...
	matplot(cbind(xts,ytso),
        ...
        box();
        axis(1);
	if (scalex == scaley | offset !=0) {
		loc <- pretty(c(xts, ytso))
        	axis(2,at=loc, labels=signif(loc/scalex, digits=2), lwd=1.5);
        	axis(4,at=loc, labels=signif(loc/scaley, digits=2), col="red", col.axis="red", lwd=1.5);
	} else {
		loc <- pretty(c(xts, ytso))
        	axis(2,at=loc, labels=signif(loc/scalex, digits=2), lwd=1.5);
        	axis(4,at=loc+offset, labels=signif(loc/scaley, digits=2), col="red", col.axis="red", lwd=1.5);
	}	
