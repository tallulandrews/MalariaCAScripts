...
ann <- read.table("~/Collaborations/MCA/For10XVelocity/pfNCscmapclusts2method_20181215.csv", sep=",", header=TRUE)
ann[,2] <- sub(".1$", "", ann[,2])

# Read in Data

emat <- readRDS("non_comp_out_S_Matrix.rds")
smat <- readRDS("non_comp_out_U_Matrix.rds")

require("Matrix")

cell_names <- read.delim("non_comp_out_ca_matrix.csv", ",", header=TRUE)
cell_names <- sub("possorted_genome_bam_W7Q5I:", "", cell_names$CellID)
cell_names <- sub("x$", "", cell_names)

gene_ann <- read.delim("non_comp_out_ra_matrix.csv", ",", header=TRUE)
...
# tmp fix diff cells
ann <- ann[ann[,2] %in% colnames(emat),]
...
saveRDS(dat, file="noncomp_pf_10X_velodata.rds")
...
saveRDS(rvel.qf, file="noncomp_pf_10X_veloEst.rds")
...
png("noncomp_falciparum_10X_veloPCA.png", width=7, height=7, units="in", res=300)
pca.out <- pca.velocity.plot(rvel.qf,nPcs=5,plot.cols=2,cex=1, show.grid.flow=TRUE, grid.n=25, cell.colors=my_colors)
dev.off()
saveRDS(pca.out, file="noncomp_pf_10X_veloPCA.rds")
