...
plot(dat_10X$ann[,c("pc1_self")]*-1, dat_10X$ann[, "pc2_self"], col=remap_colors(dat_10X$ann$cell_color), pch=16, xlab="PC1", ylab="PC2")
#points(tmp_xes[start_cell]+fitg[1,1], tmp_yes[start_cell]+fitg[2,1], col="red", pch=16)
points(c(st_x+fitg[1,1])*-1, st_y+fitg[2,1], col="red", pch=16)
lines(plot_fit[,1]*-1, plot_fit[,2], col="black", pch=16, lwd=2, lty=1)
legend("bottomleft", bty="n", "Start cell", pch=16, col="red")

arrows(plot_fit1[tick_pts,1]*-1, plot_fit1[tick_pts,2], -1*plot_fit2[tick_pts,1], plot_fit2[tick_pts,2], len=0, lwd=2)
...
lines(c(plot_fit[78,1], fitg[1,1], st_x+fitg[1,1])*-1, 
      c(plot_fit[78,2], fitg[2,1], st_y+fitg[2,1]), col="black", lwd=2) 

#text(fitg[1,]+1.75, fitg[2,]+1, expression(theta), cex=2)
text(c(fitg[1,]+0.75)*-1, fitg[2,]+0.25, expression(theta), cex=2)
points(c(st_x+fitg[1,1])*-1, st_y+fitg[2,1], col="red", pch=16)

points(bulk$PC1*-1, bulk$PC2, col="black", cex=1.5, pch=16)
...
text(bulk$PC1*-1, bulk$PC2, labels=bulk[,2], col="black", cex=1.5, font=2, pos=c(4,4,4,1))
